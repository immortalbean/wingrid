...
class Button(window.Element):
    def __init__(self, name: str,position: pygame.Vector2, size: int, text: str = '', text_color: tuple = (255, 255, 255)):
        ...
        if size < 2:
            ...
            print(f"[WinGrid error 011] Error: Buttons need to be atleast 2 tiles wide. (line {caller.lineno} in {caller.filename})",file=sys.stderr)
            sys.exit(2)
...
class Slider(window.Element):
    def __init__(self, name: str, position: pygame.Vector2, size: int, slider_range: tuple[float, float] = (0.0, 1.0), starting_value: float = 0.5):
        super().__init__(name, position)
        self.size[0] = size
...
class InternalSurface(window.Element):
    def __init__(self, name: str, position: pygame.Vector2, size: pygame.Vector2 = pygame.Vector2(2,2)):
        ...
        if min(size.x, size.y) < 2:
            caller = inspect.stack()[1]
            print(f"[WinGrid error 012] Error: Internal Surfaces must have a size of atleast 2x2. (line {caller.lineno} in {caller.filename})",file=sys.stderr)
            sys.exit(2)
        self.size = size
